...
Only the Windows leg of `test`/`build` moves — `LUMEN_RUNNER_LINUX` stays unset and keeps using a
hosted runner, so this alone does **not** get you out of an account-wide block; it only helps once
at least the Windows jobs are unblocked while Linux jobs still are, or once you register an
additional machine for the rest.
